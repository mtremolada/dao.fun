/**
 * Chain reader — spec 6.7. Runs in the BROWSER or on a server: feeds the
 * proposal view and the dashboard from CHAIN state, recomputed locally, not
 * from anything a backend stored (a decentralized app trusts no read claims):
 *
...
import { unwrap, type WrapContext } from "./execution-adapter";
...
/**
 * Browser decode source (INV-10): the UNWRAPPED inner instructions of a
 * proposal, re-read from chain and Squads-unwrapped, for client-side decoding.
 * Mirrors getProposalState's recompute path; `complete=false` means the set was
 * truncated (treat the decode as partial, same as the badge fail-safe).
 */
export async function readProposalInstructions(
  connection: Connection,
  proposal: PublicKey,
  programVersion = 3,
  programId = SPL_GOVERNANCE_PROGRAM_ID,
): Promise<{ instructions: TransactionInstruction[]; complete: boolean } | null> {
  let account;
  try {
  ...
  const expectedCount =
    ...
    account.instructionsNextIndex ??
    0;
  const { wrapped, complete } = await collectProposalTransactions(
    expectedCount,
    async (index) => {
      const addr = await getProposalTransactionAddress(
        programId,
      ...
      let pt;
      try {
        pt = await getGovernanceAccount(connection, addr, ProposalTransaction);
      } catch {
        return null;
      }
      return {
  ...
  let instructions = wrapped;
  try {
    instructions = unwrap(wrapped, NULL_CTX);
  } catch {
    // not a wrapped set — decode the raw instructions
  }
