/**
 * Chain reader — spec 6.7. Feeds the proposal view and the dashboard from
 * CHAIN state, not from anything we stored. Lives in the SDK so it runs in
 * BOTH the keyed backend and the browser: a fully client-side, serverless
 * deployment (D-033) constructs an RpcChainReader over a browser Connection
 * and reads the chain directly, with no server in the trust path.
 *
