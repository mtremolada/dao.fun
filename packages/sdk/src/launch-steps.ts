 ...
 * the step machine. Relocated to the SDK (D-033) so the launch ceremony can
 * be driven from the browser; chain access is still injected (LaunchStepDeps)
 * so this logic stays unit-tested offline and the wiring to a real Connection
 * (RpcLaunchStepDeps) lives with the caller.
 *
